...
import io.horizontalsystems.walletkit.entities.transactionrecords.evm.EvmTransactionInfo
import io.horizontalsystems.walletkit.entities.transactionrecords.evm.ApproveTransactionRecord
import io.horizontalsystems.walletkit.entities.transactionrecords.evm.ContractCallTransactionRecord
import io.horizontalsystems.walletkit.entities.transactionrecords.evm.ContractCreationTransactionRecord
import io.horizontalsystems.walletkit.entities.transactionrecords.evm.EvmIncomingTransactionRecord
import io.horizontalsystems.walletkit.entities.transactionrecords.evm.EvmOutgoingTransactionRecord
import io.horizontalsystems.walletkit.entities.transactionrecords.evm.EvmTransactionRecord
import io.horizontalsystems.walletkit.entities.transactionrecords.evm.ExternalContractCallTransactionRecord
    ...
    suspend fun transactionRecord(fullTransaction: FullTransaction): EvmTransactionRecord {
        ...
        val transactionRecord = when (val decoration = fullTransaction.decoration) {
            is ContractCreationDecoration -> {
                ContractCreationTransactionRecord(transaction.toEvmTransactionInfo(), baseToken, source, protected)
            }

            is IncomingDecoration -> {
                val fromAddress = decoration.from.eip55
                ...
                EvmIncomingTransactionRecord(transaction.toEvmTransactionInfo(), baseToken, source, fromAddress, transactionValue, isSpam, protected)
            }

            is OutgoingDecoration -> {
                val toAddress = decoration.to.eip55
                EvmOutgoingTransactionRecord(
                    transaction.toEvmTransactionInfo(),
                    baseToken,
                    source,
                    toAddress,
            ...
            is OutgoingEip20Decoration -> {
                ...
                EvmOutgoingTransactionRecord(
                    ...
                    source,
                    toAddress,
                    getEip20Value(decoration.contractAddress, decoration.value, true, decoration.tokenInfo),
                    decoration.sentToSelf,
                    protected
                )
            }
            ...
            is SwapDecoration -> {
                SwapTransactionRecord(
                    transaction.toEvmTransactionInfo(), baseToken, source,
        ...
        return transactionRecord ?: EvmTransactionRecord(
            transaction = transaction.toEvmTransactionInfo(),
            baseToken = baseToken,
            source = source,
            protected = protected,
