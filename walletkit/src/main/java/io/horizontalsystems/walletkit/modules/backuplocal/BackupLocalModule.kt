...
object BackupLocalModule {
    ...
    fun getAccountTypeString(accountType: AccountType): String = when (accountType) {
        ...
        is AccountType.NearAddress -> NEAR_ADDRESS
        ...
        is AccountType.MoneroMnemonic -> MONERO_MNEMONIC
        is AccountType.Passkey -> PASSKEY
    }
    ...
    fun getAccountTypeFromData(accountType: String, data: ByteArray): AccountType {
        return when (accountType) {
            ...
            NEAR_ADDRESS -> AccountType.NearAddress(String(data, Charsets.UTF_8))
            THORCHAIN_ADDRESS -> AccountType.ThorchainAddress(String(data, Charsets.UTF_8))
            MAYACHAIN_ADDRESS -> AccountType.MayachainAddress(String(data, Charsets.UTF_8))
            ...
            MONERO_MNEMONIC -> {
                ...
                val words = parts[0].split(" ")
                val passphrase = if (parts.size > 1) parts[1] else ""
                AccountType.MoneroMnemonic(words, passphrase)
            }
            PASSKEY -> AccountType.Passkey(String(data, Charsets.UTF_8))
    ...
    fun getDataForEncryption(accountType: AccountType): ByteArray = when (accountType) {
        ...
        is AccountType.NearAddress -> accountType.address.toByteArray(Charsets.UTF_8)
        ...
        is AccountType.MoneroMnemonic -> {
            ...
            (accountType.words.joinToString(" ") + passphrasePart).toByteArray(Charsets.UTF_8)
        }
