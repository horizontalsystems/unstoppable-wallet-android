...
class StellarTransactionsPollerTest {

    private val kitManager = mockk<StellarKitManager>(relaxed = true)
    private val transactionAdapterManager = mockk<TransactionAdapterManager>(relaxed = true)
    private val poller = StellarTransactionsPoller(kitManager, transactionAdapterManager)

    private fun mockWallet() = mockk<Wallet>(relaxed = true) {
        every { token.blockchainType } returns BlockchainType.Stellar
    }
