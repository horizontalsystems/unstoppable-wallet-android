...
class TronTransactionsPollerTest {

    private val kitManager = mockk<TronKitManager>(relaxed = true)
    private val transactionAdapterManager = mockk<TransactionAdapterManager>(relaxed = true)
    private val poller = TronTransactionsPoller(kitManager, transactionAdapterManager)

    private fun mockWallet() = mockk<Wallet>(relaxed = true) {
        every { token.blockchainType } returns BlockchainType.Tron
    }
