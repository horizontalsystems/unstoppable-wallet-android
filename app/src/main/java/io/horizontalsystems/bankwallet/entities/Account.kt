...
import io.horizontalsystems.bankwallet.entities.AccountType.Derivation.values
...
sealed class AccountType : Parcelable {
    ...
    data class Mnemonic(
        ...
        override fun hashCode(): Int = words.toTypedArray().contentHashCode() + passphrase.hashCode()
    }
    ...
    data class EvmPrivateKey(
        ...
        override fun equals(other: Any?): Boolean = other is EvmPrivateKey && key == other.key

        override fun hashCode(): Int = key.hashCode()
    }

    @Parcelize
    data class HdExtendedKey(
        val keySerialized: String,
    ) : AccountType() {
        val hdExtendedKey: HDExtendedKey
            get() = HDExtendedKey(keySerialized)

        override fun equals(other: Any?): Boolean = other is HdExtendedKey && keySerialized.contentEquals(other.keySerialized)

        override fun hashCode(): Int = keySerialized.hashCode()
    }
        ...
        val signer =
            when (this) {
                is Mnemonic -> {
                    Signer.getInstance(
                        seed,
                        App.evmBlockchainManager.getChain(BlockchainType.Ethereum),
                    )
                }
