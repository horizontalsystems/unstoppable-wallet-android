...
class TechnicalAdviceViewItemFactory(
    ...
    private fun mainAdvice(technicalAdvice: TechnicalAdvice): String {
        val (overtype, direction, rsiLine) =
            when (technicalAdvice.advice ?: Advice.Neutral) {
                Advice.Oversold, Advice.StrongBuy, Advice.Buy ->
                    Triple(
                ...
                Advice.Overbought, Advice.StrongSell, Advice.Sell, Advice.Neutral ->
                    Triple(
        ...
        val rsiValue =
            technicalAdvice.rsi?.let {
                numberFormatter.format(it, 0, 1, suffix = "%")
            }

        val signalTimeString =
            technicalAdvice.signalTimestamp
                ?.let { timestamp ->
        ...
        val adviceString =
            when (technicalAdvice.advice) {
                Advice.Oversold, Advice.Overbought -> {
                    ...
                    val combinedIndicators =
                        if (signalTimeString != null) {
                            listOf(
                                signalTimeString,
                                indicators,
                            ).joinToString(" ")
