...
class YakAuthorizationService(
    ...
    sealed class State {
        ...
        class SignMessageReceived(
            val accountData: ProFeaturesAuthorizationManager.AccountData,
            val message: String,
        ) : State()
    ...
    suspend fun signConfirmed() =
        withContext(Dispatchers.IO) {
            val messageReceivedState =
                _stateFlow.value as? State.SignMessageReceived ?: return@withContext

            try {
                val signature =
                    manager.signMessage(messageReceivedState.accountData, messageReceivedState.message)

                val key =
                    adapter.authenticate(
                        messageReceivedState.accountData.address.hex,
                        "0x${signature.toRawHexString()}",
                    )
