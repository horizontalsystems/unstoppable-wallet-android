...
class TransactionInfoFragment : BottomSheetDialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.explorerButton.observe(this, Observer { (explorerName, enabled) ->
            txtViewOnExplorer.text =
                getString(R.string.TransactionInfo_ButtonViewOnExplorerName, explorerName)
            txtViewOnExplorer.isEnabled = enabled
        })

        viewModel.showCopiedLiveEvent.observe(this, Observer {
            snackBar = HudHelper.showSuccessMessage(
                this.requireView(),
        ...
        viewModel.titleLiveData.observe(this, Observer { titleViewItem ->
            val title =
                if (titleViewItem.type is TransactionType.Approve) R.string.TransactionInfo_Approval else R.string.TransactionInfo_Transaction
...
//            sentToSelfIcon.isVisible = titleViewItem.type == TransactionType.SentToSelf
            ...
            primaryValue.setCompoundDrawablesWithIntrinsicBounds(
                0,
                0,
                TransactionViewHelper.getLockIcon(titleViewItem.lockState),
                0
            )
            primaryValue.setTextColor(
                TransactionViewHelper.getAmountColor(
                    titleViewItem.type,
                    requireContext()
                )
            )
