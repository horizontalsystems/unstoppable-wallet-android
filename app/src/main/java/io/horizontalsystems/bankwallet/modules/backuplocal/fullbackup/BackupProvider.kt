...
class BackupProvider(
    ...
    fun restoreCexAccount(
        accountType: AccountType,
        accountName: String,
    ) {
        val account =
            accountFactory.account(accountName, accountType, AccountOrigin.Restored, true, true)
    ...
    private fun restoreChartSettings(
        ...
        val rsiChartSettings =
            rsiDefaults.sortedBy { it.index }.take(rsi.size).map { default ->
        ...
        val maChartSettings =
            maDefaults.sortedBy { it.index }.take(ma.size).map { default ->
        ...
        val macdChartSettings =
            macdDefaults.sortedBy { it.index }.take(macd.size).map { default ->
    ...
    private fun fullBackupItems(
        ...
        val nonWatchAccounts =
            accounts.filter { !it.isWatchAccount }.sortedBy { it.name.lowercase() }
    ...
    fun fullBackupItems(decryptedFullBackup: DecryptedFullBackup) =
        fullBackupItems(
            ...
            customRpcsCount =
                decryptedFullBackup.settings.evmSyncSources.custom
                    .ifEmpty { null }
                    ?.size,
        )

    fun shouldShowReplaceWarning(decryptedFullBackup: DecryptedFullBackup?): Boolean =
        decryptedFullBackup != null && decryptedFullBackup.contacts.isNotEmpty() && contactsRepository.contacts.isNotEmpty()

    @Throws
    fun createWalletBackup(
        account: Account,
        passphrase: String,
    ): String {
    ...
    fun createFullBackup(
        ...
        val contacts =
            if (contactsRepository.contacts.isNotEmpty()) {
...
sealed class RestoreException(
    ...
    object InvalidPasswordException :
        RestoreException(Translator.getString(R.string.ImportBackupFile_Error_InvalidPassword))
}
