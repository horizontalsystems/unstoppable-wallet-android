...
class SendEvmTransactionViewModel(
    ...
    private fun getTransferViewItems(transfer: TransactionDecoration.Transfer, additionalInfo: SendEvmData.AdditionalInfo?): List<SectionViewItem> {
        val viewItems = mutableListOf(
                ViewItem.Subhead(
                        translator.string(R.string.Send_Confirmation_YouSend),
                        coinServiceFactory.baseCoinService.coin.title
                ),
                ViewItem.Value(
                        translator.string(R.string.Send_Confirmation_Amount),
        ...
        viewItems.add(ViewItem.Address(
                translator.string(R.string.Send_Confirmation_To),
                addressTitle,
                value = addressValue)
        )
    ...
    private fun getEip20TransferViewItems(eip20Transfer: TransactionDecoration.Eip20Transfer, additionalInfo: SendEvmData.AdditionalInfo?): List<SectionViewItem>? =
            coinServiceFactory.getCoinService(eip20Transfer.contractAddress)?.let { coinService ->
                val viewItems = mutableListOf(
                        ViewItem.Subhead(translator.string(R.string.Send_Confirmation_YouSend), coinService.coin.title),
                        ViewItem.Value(translator.string(R.string.Send_Confirmation_Amount), coinService.amountData(eip20Transfer.value).getFormatted(), ValueType.Outgoing)
                )
                ...
                viewItems.add(
                        ViewItem.Address(translator.string(R.string.Send_Confirmation_To), addressTitle, value = addressValue)
                )
    ...
    private fun getEip20ApproveViewItems(eip20Approve: TransactionDecoration.Eip20Approve): List<SectionViewItem>? =
            coinServiceFactory.getCoinService(eip20Approve.contractAddress)?.let { coinService ->
                ...
                val viewItems = listOf(
                        ViewItem.Subhead(translator.string(R.string.Approve_YouApprove), coinService.coin.title),
                        ViewItem.Value(translator.string(R.string.Send_Confirmation_Amount), coinService.amountData(eip20Approve.value).getFormatted(), ValueType.Regular),
                        ViewItem.Address(translator.string(R.string.Approve_Spender), addressTitle, addressValue)
                )
                listOf(SectionViewItem(viewItems))
            }
    ...
    private fun getSwapViewItems(swap: TransactionDecoration.Swap, additionalInfo: SendEvmData.AdditionalInfo?): List<SectionViewItem>? {
        ...
        when (val trade = swap.trade) {
            is TransactionDecoration.Swap.Trade.ExactIn -> {
                sections.add(SectionViewItem(listOf(
                        ViewItem.Subhead(translator.string(R.string.Swap_FromAmountTitle), coinServiceIn.coin.title),
                        ViewItem.Value(translator.string(R.string.Send_Confirmation_Amount), coinServiceIn.amountData(trade.amountIn).getFormatted(), ValueType.Outgoing)
                )))
                sections.add(SectionViewItem(listOf(
                        ViewItem.Subhead(translator.string(R.string.Swap_ToAmountTitle), coinServiceOut.coin.title),
                        getEstimatedSwapAmount(info?.let { coinServiceOut.amountData(it.estimatedOut).getFormatted() }, ValueType.Incoming),
                        ViewItem.Value(translator.string(R.string.Swap_Confirmation_Guaranteed), coinServiceOut.amountData(trade.amountOutMin).getFormatted(), ValueType.Regular)
                )))
            }
            is TransactionDecoration.Swap.Trade.ExactOut -> {
                sections.add(SectionViewItem(listOf(
                        ...
                        ViewItem.Value(translator.string(R.string.Swap_Confirmation_Maximum), coinServiceIn.amountData(trade.amountInMax).getFormatted(), ValueType.Regular)
                )))
                sections.add(SectionViewItem(listOf(
                        ViewItem.Subhead(translator.string(R.string.Swap_ToAmountTitle), coinServiceOut.coin.title),
                        ViewItem.Value(translator.string(R.string.Swap_Confirmation_Guaranteed), coinServiceOut.amountData(trade.amountOut).getFormatted(), ValueType.Regular)
                )))
            }
        }
        ...
        info?.slippage?.let {
            otherViewItems.add(ViewItem.Value(translator.string(R.string.SwapSettings_SlippageTitle), it, ValueType.Regular))
        }
        info?.deadline?.let {
            otherViewItems.add(ViewItem.Value(translator.string(R.string.SwapSettings_DeadlineTitle), it, ValueType.Regular))
        }
        if (swap.to != service.ownAddress) {
            val addressValue = swap.to.eip55
            val addressTitle = info?.recipientDomain
                    ?: TransactionInfoAddressMapper.map(addressValue)
            otherViewItems.add(ViewItem.Address(translator.string(R.string.SwapSettings_RecipientAddressTitle), addressTitle, addressValue))
        }
        info?.price?.let {
            otherViewItems.add(ViewItem.Value(translator.string(R.string.Swap_Price), it, ValueType.Regular))
        }
        info?.priceImpact?.let {
            otherViewItems.add(ViewItem.Value(translator.string(R.string.Swap_PriceImpact), it, ValueType.Regular))
        }
    ...
    private fun getEstimatedSwapAmount(value: String?, type: ValueType): ViewItem {
        val title = translator.string(R.string.Swap_Confirmation_Estimated)
        return value?.let { ViewItem.Value(title, it, type) }
                ?: ViewItem.Value(title, translator.string(R.string.NotAvailable), ValueType.Disabled)
    }
    ...
    private fun getFallbackViewItems(transactionData: TransactionData): List<SectionViewItem> {
        ...
        val viewItems = listOf(
                ViewItem.Value(translator.string(R.string.Send_Confirmation_Amount), coinServiceFactory.baseCoinService.amountData(transactionData.value).getFormatted(), ValueType.Outgoing),
                ViewItem.Address(translator.string(R.string.Send_Confirmation_To), addressValue, addressValue),
    ...
    private fun convertError(error: Throwable) =
            when (val convertedError = error.convertedError) {
                is SendEvmTransactionService.TransactionError.InsufficientBalance -> {
                    translator.string(R.string.EthereumTransaction_Error_InsufficientBalance, coinServiceFactory.baseCoinService.coinValue(convertedError.requiredBalance).getFormatted())
                }
                is EvmError.InsufficientBalanceWithFee,
                is EvmError.ExecutionReverted -> {
                    translator.string(R.string.EthereumTransaction_Error_InsufficientBalanceForFee, coinServiceFactory.baseCoinService.coin.code)
                }
