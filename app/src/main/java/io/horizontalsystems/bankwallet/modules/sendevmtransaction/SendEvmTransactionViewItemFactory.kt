...
class SendEvmTransactionViewItemFactory(
    ...
    private fun getViewItems(
        ...
        when (decoration) {
            is OutgoingDecoration ->
                getSendBaseCoinItems(
                    decoration.to,
                    decoration.value,
                )

            is OutgoingEip20Decoration ->
                getEip20TransferViewItems(
                    decoration.to,
                    decoration.value,
    ...
    private fun getOneInchSwapViewItems(
        ...
        val outViewItems: MutableList<ViewItem> =
            mutableListOf(
                ViewItem.Subhead(
                    Translator.getString(R.string.Swap_ToAmountTitle),
                    coinServiceOut.token.coin.name,
    ...
    private fun getEip20TransferViewItems(
        ...
        val viewItems: MutableList<ViewItem> =
            mutableListOf(
                getAmountWithTitle(
    ...
    private fun getContact(addressValue: String): Contact? =
        contactsRepo
            .getContactsFiltered(blockchainType, addressQuery = addressValue)
            .firstOrNull()
    ...
    private fun getSendBaseCoinItems(
        to: Address,
        value: BigInteger,
    ): List<SectionViewItem> {
    ...
    private fun getCoinService(token: SwapDecoration.Token) =
        when (token) {
    ...
    private fun getCoinService(token: OneInchDecoration.Token) =
        when (token) {
    ...
    ): ViewItem.NftAmount = ViewItem.NftAmount(previewImageUrl, "$value NFT", ValueType.Regular)
