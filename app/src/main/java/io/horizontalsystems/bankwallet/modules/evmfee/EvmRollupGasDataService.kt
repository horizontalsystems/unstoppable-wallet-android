...
class EvmRollupGasDataService(
    ...
    override fun estimatedGasDataAsync(
        gasPrice: GasPrice,
        transactionData: TransactionData,
        stubAmount: BigInteger?,
    ): Single<GasData> =
        if (predefinedGasLimit != null) {
            ...
            super.estimatedGasDataAsync(gasPrice, transactionData, stubAmount).flatMap { gasData ->
                ...
                val stubTransactionData =
                    if (stubAmount != null) {
                        TransactionData(
                            transactionData.to,
                            maxBytes(transactionData.value),
    ...
    private fun l1GasFee(
        transactionData: TransactionData,
        gasPrice: GasPrice,
        gasLimit: Long,
    ): Single<BigInteger> =
        l1FeeProvider.getL1Fee(
            gasPrice,
            gasLimit,
            transactionData.to,
