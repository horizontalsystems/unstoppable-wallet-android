...
object SendModule {
    data class AmountData(
        val primary: AmountInfo.CoinValueInfo,
        val secondary: AmountInfo.CurrencyValueInfo?,
    )

    sealed class AmountInfo {
        abstract val approximate: Boolean

        data class CoinValueInfo(
            val coinValue: CoinValue,
            override val approximate: Boolean = false,
        ) : AmountInfo()

        data class CurrencyValueInfo(
...
sealed class SendResult {
    ...
    class Sent(
        val transactionRecord: TransactionRecord? = null,
    ) : SendResult()
...
class SendErrorInsufficientBalance(
    coinCode: Any,
) : HSCaution(
