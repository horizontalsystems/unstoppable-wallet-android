...
import io.horizontalsystems.bankwallet.modules.settings.security.autolock.AutoLockInterval.values

object AutoLockModule {
    class Factory : ViewModelProvider.Factory {
        @Suppress("UNCHECKED_CAST")
        override fun <T : ViewModel> create(modelClass: Class<T>): T = AutoLockIntervalsViewModel(App.localStorage) as T
    }

    data class AutoLockIntervalViewItem(
        val interval: AutoLockInterval,
        val selected: Boolean,
    )
}

enum class AutoLockInterval(
    val raw: String,
    val title: Int,
) {
    ...
    companion object {
        fun fromRaw(raw: String): AutoLockInterval? = values().find { it.raw == raw }
    }
}
