...
class TotalBalance(
    ...
    private fun getPrimaryAmount(totalState: TotalService.State.Visible) =
        totalState.currencyValue?.let {
            App.numberFormatter.formatFiatShort(it.value, it.currency.symbol, 2)
        }

    private fun getSecondaryAmount(totalState: TotalService.State.Visible) =
