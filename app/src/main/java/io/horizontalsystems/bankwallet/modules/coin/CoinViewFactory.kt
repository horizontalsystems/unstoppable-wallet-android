...
class CoinViewFactory(
    ...
    private fun getMarketItems(item: CoinOverviewItem): MutableList<CoinDataItem> {
        val overview = item.marketInfoOverview
    ...
    fun getLinks(
        coinMarketDetails: MarketInfoOverview,
        guideUrl: String?,
    ): List<CoinLink> {
        val linkTypes =
            listOf(
...
//            LinkType.YOUTUBE -> Translator.getString(R.string.CoinPage_Youtube)
    ...
    private fun formatFiatShortened(
        value: BigDecimal,
        symbol: String,
    ): String = numberFormatter.formatFiatShort(value, symbol, 2)
}
