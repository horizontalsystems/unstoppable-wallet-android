...
open class TransactionInfoStatusView : ConstraintLayout {
    ...
    private fun fillProgress(transactionStatus: TransactionStatus.Processing) {
        ...
        if (transactionStatus.progress == 1) return
        progressBar2.setImageResource(greenBar)
        if (transactionStatus.progress == 2) return
        progressBar3.setImageResource(greenBar)
        if (transactionStatus.progress == 3) return
        progressBar4.setImageResource(greenBar)
        if (transactionStatus.progress == 4) return
        progressBar5.setImageResource(greenBar)
        if (transactionStatus.progress == 5) return
