...
import io.horizontalsystems.bankwallet.modules.market.MarketField.values
import io.horizontalsystems.bankwallet.modules.market.MarketModule.Tab.entries
import io.horizontalsystems.bankwallet.modules.market.SortingField.values
import io.horizontalsystems.bankwallet.modules.market.TopMarket.entries
...
fun List<MarketItem>.sort(sortingField: SortingField) =
    when (sortingField) {
...
inline fun <T, R : Comparable<R>> Iterable<T>.sortedByDescendingNullLast(crossinline selector: (T) -> R?): List<T> =
    sortedWith(compareBy(nullsFirst(), selector)).sortedByDescending(selector)

inline fun <T, R : Comparable<R>> Iterable<T>.sortedByNullLast(crossinline selector: (T) -> R?): List<T> =
    sortedWith(compareBy(nullsLast(), selector))

fun MarketInfo.priceChangeValue(period: TimePeriod) =
    when (period) {
