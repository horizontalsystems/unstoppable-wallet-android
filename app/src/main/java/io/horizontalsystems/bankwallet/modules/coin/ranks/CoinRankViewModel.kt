...
class CoinRankViewModel(
    ...
    private val periodOptions =
        listOf(TimeDuration.OneDay, TimeDuration.SevenDay, TimeDuration.ThirtyDay)
        ...
        viewModelScope.launch {
            rankViewItems =
                withContext(Dispatchers.IO) {
                    val items =
                        internalItems.mapNotNull { internalItem ->
                            val resolvedValue: BigDecimal? =
                                when (val anyValue = internalItem.value) {
                    ...
                    val viewItems =
                        topItems.mapIndexed { index, item ->
    ...
    private suspend fun getRank(
        ...
        when (rankType) {
            ...
            RankType.TransactionCountRank ->
                marketKit
                    .transactionCountsRanksSingle(currencyCode)
                    .await()
    ...
    private fun formatted(
        ...
        when (rankType) {
            ...
            RankType.TransactionCountRank,
            -> numberFormatter.formatNumberShort(value, 0)
        }

    private fun getPeriodMenu(): Select<TimeDuration>? =
        when (rankType) {
            RankType.DexLiquidityRank,
            RankType.HoldersRank,
            -> null
