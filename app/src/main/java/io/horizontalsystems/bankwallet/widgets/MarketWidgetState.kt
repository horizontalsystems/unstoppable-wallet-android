...
import io.horizontalsystems.bankwallet.widgets.MarketWidgetType.values
...
data class MarketWidgetItem(
    ...
    override fun toString(): String =
        "( title: $title, subtitle: $subtitle, label: $label, value: $value, diff: $diff, imageRemoteUrl: $imageRemoteUrl, imageLocalPath: $imageLocalPath )"
}

class MarketWidgetTypeAdapter : TypeAdapter<MarketWidgetType>() {
    override fun write(
        writer: JsonWriter,
        value: MarketWidgetType,
    ) {
        writer.value(value.id)
    }

    override fun read(reader: JsonReader): MarketWidgetType? = MarketWidgetType.fromId(reader.nextString())
}
