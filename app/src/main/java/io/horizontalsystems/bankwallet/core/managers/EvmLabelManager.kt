...
class EvmLabelManager(
    ...
    private fun addressLabel(address: String): String? = addressLabelDao.get(address.lowercase())?.label

    fun mapped(address: String): String = addressLabel(address) ?: address.shorten()

    private suspend fun syncAddressLabels(timestamp: Long) {
        val lastSyncTimestamp =
            syncerStateStorage.get(keyAddressLabelsTimestamp)?.value?.toLongOrNull()
    ...
    private suspend fun syncMethodLabels(timestamp: Long) {
        val lastSyncTimestamp =
            syncerStateStorage.get(keyMethodLabelsTimestamp)?.value?.toLongOrNull()
        if (lastSyncTimestamp == timestamp) return

        val methodLabels = provider.evmMethodLabels()
        methodLabelDao.update(
            methodLabels.map {
                EvmMethodLabel(
                    it.methodId.lowercase(),
                    it.label,
                )
            },
        )
