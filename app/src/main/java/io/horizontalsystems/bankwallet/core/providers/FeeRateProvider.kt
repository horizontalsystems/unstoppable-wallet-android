...
class FeeRateProvider(
    ...
    fun bitcoinFeeRate(): Single<MempoolSpaceProvider.RecommendedFees> = feeRateKit.bitcoin()

    fun litecoinFeeRate(): Single<BigInteger> = feeRateKit.litecoin()

    fun bitcoinCashFeeRate(): Single<BigInteger> = feeRateKit.bitcoinCash()

    fun dashFeeRate(): Single<BigInteger> = feeRateKit.dash()
}

class BitcoinFeeRateProvider(
...
class LitecoinFeeRateProvider(
    ...
    override suspend fun getFeeRates(): FeeRates = FeeRates(2)
}

class BitcoinCashFeeRateProvider(
    private val feeRateProvider: FeeRateProvider,
) : IFeeRateProvider {
    override suspend fun getFeeRates(): FeeRates {
