...
object Migration_31_32 : Migration(31, 32) {
    ...
    private fun handleZcashAccount(database: SupportSQLiteDatabase) {
        ...
        if (idColumnIndex >= 0 && birthdayHeightColumnIndex >= 0) {
            while (zcashAccountsCursor.moveToNext()) {
                ...
                database.execSQL(
                    "INSERT INTO `RestoreSettingRecord`(`accountId`, `coinId`, `key`, `value`) VALUES('$id', 'zcash', 'birthdayHeight', '$birthdayHeight')",
                )
            }
        }
    ...
    private fun updateAccountRecordTable(database: SupportSQLiteDatabase) {
        ...
        database.execSQL(
            "CREATE TABLE IF NOT EXISTS `AccountRecord` (`deleted` INTEGER NOT NULL, `id` TEXT NOT NULL, `name` TEXT NOT NULL, `type` TEXT NOT NULL, `origin` TEXT NOT NULL, `isBackedUp` INTEGER NOT NULL, `words` TEXT, `passphrase` TEXT, `key` TEXT, PRIMARY KEY(`id`))",
        )
        database.execSQL(
            "INSERT INTO AccountRecord (`deleted`, `id`, `name`, `type`, `origin`, `isBackedUp`, `words`, `passphrase`, `key`) SELECT `deleted`, `id`, `name`, `type`, `origin`, `isBackedUp`, `words`, `salt`, `key` FROM TempAccountRecord",
        )
    ...
    private fun createTableActiveAccount(database: SupportSQLiteDatabase) {
        database.execSQL(
            "CREATE TABLE IF NOT EXISTS `ActiveAccount` (`accountId` TEXT NOT NULL, `primaryKey` TEXT NOT NULL, PRIMARY KEY(`primaryKey`))",
        )
    }

    private fun createTableRestoreSettings(database: SupportSQLiteDatabase) {
        database.execSQL(
            "CREATE TABLE IF NOT EXISTS `RestoreSettingRecord` (`accountId` TEXT NOT NULL, `coinId` TEXT NOT NULL, `key` TEXT NOT NULL, `value` TEXT NOT NULL, PRIMARY KEY(`accountId`, `coinId`, `key`))",
        )
    }
    ...
    private fun moveCoinSettingsFromBlockchainSettingsToWallet(database: SupportSQLiteDatabase) {
        ...
        database.execSQL(
            "CREATE TABLE IF NOT EXISTS `EnabledWallet` (`coinId` TEXT NOT NULL, `coinSettingsId` TEXT NOT NULL, `accountId` TEXT NOT NULL, `walletOrder` INTEGER, PRIMARY KEY(`coinId`, `coinSettingsId`, `accountId`), FOREIGN KEY(`accountId`) REFERENCES `AccountRecord`(`id`) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED)",
        )
        database.execSQL(
            "INSERT INTO EnabledWallet (`coinId`, `coinSettingsId`, `accountId`, `walletOrder`) SELECT `coinId`,'',`accountId`,`walletOrder` FROM TempEnabledWallet",
        )
    ...
    private fun setActiveAccount(database: SupportSQLiteDatabase) {
        val firstAccountCursor =
            database.query("SELECT id FROM AccountRecord ORDER BY rowid LIMIT 1")
