...
import io.horizontalsystems.bankwallet.core.managers.RestoreSettingType.values
...
class RestoreSettingsManager(
    ...
    fun accountSettingsInfo(account: Account): List<Triple<BlockchainType, RestoreSettingType, String>> =
        storage.restoreSettings(account.id).mapNotNull { record ->
    ...
    fun save(
        ...
        val records =
            settings.values.map { (type, value) ->
        ...
        when (settingType) {
            RestoreSettingType.BirthdayHeight ->
                Translator.getString(
                    R.string.ManageAccount_BirthdayHeight,
                    token.coin.code,
                )
        }
}
...
class RestoreSettings {
    ...
    operator fun get(key: RestoreSettingType): String? = values[key]

    operator fun set(
        key: RestoreSettingType,
        value: String,
    ) {
        values[key] = value
    }
