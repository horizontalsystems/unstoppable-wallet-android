...
class OpenSeaService(
    ...
    private val hsService by lazy {
        APIClient
            .build("$hsBaseUrl/v1/nft/", mapOf("apikey" to apiKey))
    ...
    suspend fun collection(uid: String): OpenSeaNftApiResponse.Collection = service.collection(uid).collection

    suspend fun asset(
        contractAddress: String,
        tokenId: String,
    ): OpenSeaNftApiResponse.Asset = service.asset(contractAddress, tokenId)

    suspend fun collectionAssets(
        uid: String,
        cursor: String?,
    ): OpenSeaNftApiResponse.Assets = service.assets(collectionUid = uid, cursor = cursor, limit = 30)

    suspend fun collectionEvents(
        uid: String,
        type: String?,
        cursor: String?,
    ): OpenSeaNftApiResponse.Events = hsService.events(collectionUid = uid, eventType = type, cursor = cursor)
    ...
    ): OpenSeaNftApiResponse.Assets = service.assets(contractAddresses = contractAddresses, tokenIds = tokenIds)
}
