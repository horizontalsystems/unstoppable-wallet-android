...
class NftMetadataManager(
    ...
    private val _addressMetadataFlow =
        MutableSharedFlow<Pair<NftKey, NftAddressMetadata>?>(
            replay = 1,
            onBufferOverflow = BufferOverflow.DROP_OLDEST,
        )
    val addressMetadataFlow: Flow<Pair<NftKey, NftAddressMetadata>?> = _addressMetadataFlow

    suspend fun addressMetadata(
        blockchainType: BlockchainType,
        address: String,
    ): NftAddressMetadata = provider(blockchainType).addressMetadata(blockchainType, address)

    fun addressMetadata(nftKey: NftKey): NftAddressMetadata? = storage.addressInfo(nftKey)

    fun handle(
        nftAddressMetadata: NftAddressMetadata,
        nftKey: NftKey,
    ) {
    ...
    fun provider(blockchainType: BlockchainType): INftProvider =
        providerMap[blockchainType] ?: throw ProviderError.NoProviderForBlockchainType(
            blockchainType,
        )
    ...
    fun assetShortMetadata(nftUid: NftUid): NftAssetShortMetadata? = storage.assetShortMetadata(nftUid)

    fun assetsBriefMetadata(nftUids: Set<NftUid>): List<NftAssetBriefMetadata> = storage.assetsBriefMetadata(nftUids)
    ...
    sealed class ProviderError(
        ...
        class NoProviderForBlockchainType(
            blockchainType: BlockchainType,
