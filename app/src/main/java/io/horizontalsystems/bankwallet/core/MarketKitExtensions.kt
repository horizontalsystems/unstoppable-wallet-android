...
val Token.typeInfo: String
    get() =
        when (val type = type) {
            ...
            TokenType.Native,
            -> Translator.getString(R.string.CoinPlatforms_Native)
    ...
    when (accountType) {
        ...
        is AccountType.HdExtendedKey -> {
            ...
            when (this) {
                ...
                BlockchainType.ECash,
                ->
                    coinTypes.contains(ExtendedKeyCoinType.Bitcoin) &&
...
val Token.badge: String?
    get() =
        when (val tokenType = type) {
            is TokenType.Derived -> {
                tokenType.derivation.accountTypeDerivation.value
                    .uppercase()
            }

            is TokenType.AddressTyped -> {
                tokenType.type.bitcoinCashCoinType.value
                    .uppercase()
            }
