...
class NftEventsProvider(
    ...
    private val service: ReservoirApi =
        APIClient.retrofit(apiURL, 60).create(ReservoirApi::class.java)
    ...
    private fun events(
        ...
        return activities.map { activity ->
            val amount: NftPrice? =
                activity.price?.let { price -> token?.let { token -> NftPrice(token, price) } }
    ...
    private fun eventTypeString(eventType: NftEventMetadata.EventType?): String? =
        when (eventType) {
    ...
    private interface ReservoirApi {
        ...
        suspend fun collectionActivity(
            @Query("collection") contractAddress: String,
            @Query("types") eventType: String?,
            @Query("continuation") cursor: String?,
        ): ActivityResponse
    }
