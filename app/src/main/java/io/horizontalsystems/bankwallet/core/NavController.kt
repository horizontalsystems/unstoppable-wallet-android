...
fun NavController.slideFromRight(
    @IdRes resId: Int,
    input: Parcelable? = null,
    xxx: NavOptions.Builder.() -> Unit = { },
) {
    val builder =
        NavOptions
            .Builder()
...
fun NavController.slideFromBottom(
    @IdRes resId: Int,
    input: Parcelable? = null,
) {
    val navOptions =
...
fun NavController.paidAction(
    paidAction: IPaidAction,
    block: () -> Unit,
) {
...
inline fun <reified T : Parcelable> NavController.getInput(): T? = currentBackStackEntry?.arguments?.getInputX()

inline fun <reified T : Parcelable> Bundle.getInputX(): T? = parcelable("input")

inline fun <reified T : Parcelable> NavController.requireInput(): T = getInput()!!
