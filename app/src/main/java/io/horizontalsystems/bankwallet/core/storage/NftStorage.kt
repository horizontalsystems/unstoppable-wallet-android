...
class NftStorage(
    ...
    fun addressInfo(nftKey: NftKey): NftAddressMetadata? =
        try {
            ...
            val priceRecords =
                collectionRecords.mapNotNull { it.averagePrice7d } + collectionRecords.mapNotNull { it.averagePrice30d } +
                    assetRecords.mapNotNull { it.lastSale }
    ...
    fun save(
        ...
        val collectionRecords =
            addressMetadata.collections.map {
                NftCollectionRecord(
    ...
    fun lastSyncTimestamp(nftKey: NftKey): Long? =
        try {
    ...
    fun save(
        ...
        val record =
            NftMetadataSyncRecord(nftKey.blockchainType, nftKey.account.id, lastSyncTimestamp)
    ...
    fun save(assetsBriefMetadata: List<NftAssetBriefMetadata>) =
        try {
            nftDao.insertNftAssetBriefMetadataRecords(
                assetsBriefMetadata.map {
                    NftAssetBriefMetadataRecord(
                        it.nftUid,
                        it.providerCollectionUid,
                        it.name,
                        it.imageUrl,
                        it.previewImageUrl,
                    )
