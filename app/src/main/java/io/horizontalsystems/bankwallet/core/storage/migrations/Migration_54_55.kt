    ...
    override fun migrate(db: SupportSQLiteDatabase) {
        ...
        blockchainTypes.forEach { blockchainTypeId ->
            derivations.forEach { derivation ->
                ...
                val newTokenQueryId =
                    "$blockchainTypeId|derived:${derivation.replaceFirstChar(Char::titlecase)}"

                db.execSQL(
                    "UPDATE `EnabledWallet` SET tokenQueryId = '$newTokenQueryId' WHERE tokenQueryId = '$tokenQueryId' AND coinSettingsId = '$coinSettingsId'",
                )
            }
        }
        ...
        bchTypes.forEach { bchType ->
            ...
            val newTokenQueryId =
                "bitcoin-cash|address_type:${bchType.replaceFirstChar(Char::titlecase)}"
        ...
        db.execSQL(
            "INSERT INTO `EnabledWallet`(tokenQueryId, accountId, walletOrder, coinName, coinCode, coinDecimals) SELECT tokenQueryId, accountId, walletOrder, coinName, coinCode, coinDecimals FROM `TempEnabledWallet`",
        )
        ...
        db.execSQL(
            "CREATE TABLE IF NOT EXISTS `EnabledWalletCache` (`tokenQueryId` TEXT NOT NULL, `accountId` TEXT NOT NULL, `balance` TEXT NOT NULL, `balanceLocked` TEXT NOT NULL, PRIMARY KEY(`tokenQueryId`, `accountId`), FOREIGN KEY(`accountId`) REFERENCES `AccountRecord`(`id`) ON UPDATE CASCADE ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED)",
        )
    }
}
