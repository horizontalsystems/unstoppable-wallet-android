...
class AdapterFactory(
    ...
    private fun getEvmAdapter(wallet: Wallet): IAdapter? {
        ...
        val evmKitWrapper =
            evmBlockchainManager.getEvmKitManager(blockchainType).getEvmKitWrapper(
                wallet.account,
                blockchainType,
            )
    ...
    fun getAdapterOrNull(wallet: Wallet) =
        try {
            getAdapter(wallet)
        } catch (e: Throwable) {
    ...
    fun tronTransactionsAdapter(source: TransactionSource): ITransactionsAdapter? {
        ...
        val tronTransactionConverter =
            TronTransactionConverter(
                coinManager,
                tronKitWrapper,
                source,
                baseToken,
