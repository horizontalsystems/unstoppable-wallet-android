...
class BlockchainSettingsStorage(
    ...
    fun btcRestoreMode(blockchainType: BlockchainType): BtcRestoreMode? =
        dao.getBlockchainSetting(blockchainType.uid, keyBtcRestore)?.let { storedSetting ->
            BtcRestoreMode.values().firstOrNull { it.raw == storedSetting.value }
        }

    fun save(
        btcRestoreMode: BtcRestoreMode,
    ...
    fun btcTransactionSortMode(blockchainType: BlockchainType): TransactionDataSortMode? =
        dao
            .getBlockchainSetting(blockchainType.uid, keyBtcTransactionSort)
            ?.let { sortSetting ->
                TransactionDataSortMode.values().firstOrNull { it.raw == sortSetting.value }
            }

    fun save(
        transactionDataSortMode: TransactionDataSortMode,
        blockchainType: BlockchainType,
    ) {
        dao.insert(
            BlockchainSettingRecord(
    ...
    fun evmSyncSourceUrl(blockchainType: BlockchainType): String? = dao.getBlockchainSetting(blockchainType.uid, keyEvmSyncSourceUrl)?.value

    fun save(
        evmSyncSourceUrl: String,
        blockchainType: BlockchainType,
    ) {
        dao.insert(
            BlockchainSettingRecord(
