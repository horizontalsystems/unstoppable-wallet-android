...
class MagicEdenService(
    ...
    private val service by lazy {
        ...
        ).create(MagicEdenApi::class.java)
    }

    private val hsService by lazy {
        APIClient.build("${hsBaseUrl}/v1/nft/", mapOf("apikey" to apiKey))
                .create(MagicEdenHsNftApi::class.java)
    }

    suspend fun allCollections(address: String? = null): List<MagicEdenNftApiResponse.Collection> {
        val collections = mutableListOf<MagicEdenNftApiResponse.Collection>()
        val limit = 300
        do {
    ...
    suspend fun allAssets(address: String): List<MagicEdenNftApiResponse.Asset> {
        val assets = mutableListOf<MagicEdenNftApiResponse.Asset>()
    ...
    suspend fun collection(uid: String): MagicEdenNftApiResponse.Collection =
            service.collection(uid).collection

    suspend fun asset(contractAddress: String, tokenId: String): MagicEdenNftApiResponse.Asset =
            service.asset(contractAddress, tokenId)

    suspend fun collectionAssets(uid: String, cursor: String?): MagicEdenNftApiResponse.Assets =
            service.assets(collectionUid = uid, cursor = cursor, limit = 30)

    suspend fun collectionEvents(uid: String, type: String?, cursor: String?): MagicEdenNftApiResponse.Events =
            hsService.events(collectionUid = uid, eventType = type, cursor = cursor)

    suspend fun assetEvents(contractAddress: String, tokenId: String, type: String?, cursor: String?): MagicEdenNftApiResponse.Events =
            hsService.events(contractAddress = contractAddress, tokenId = tokenId, eventType = type, cursor = cursor)

    suspend fun assets(contractAddresses: List<String>, tokenIds: List<String>): MagicEdenNftApiResponse.Assets =
            service.assets(contractAddresses = contractAddresses, tokenIds = tokenIds)
}

interface MagicEdenApi {
    ...
    ): List<MagicEdenNftApiResponse.Collection>

    @GET("collection/{slug}")
    suspend fun collection(
            @Path("slug") slug: String,
            @Query("format") format: String = "json"
    ): MagicEdenNftApiResponse.CollectionResponse
    ...
    suspend fun assets(
            ...
            @Query("format") format: String = "json"
    ): MagicEdenNftApiResponse.Assets
    ...
    suspend fun asset(
            ...
            @Query("include_orders") includeOrders: Boolean = true
    ): MagicEdenNftApiResponse.Asset
}

interface MagicEdenHsNftApi {
    ...
    ): MagicEdenNftApiResponse.Events
}

object MagicEdenNftApiResponse {
