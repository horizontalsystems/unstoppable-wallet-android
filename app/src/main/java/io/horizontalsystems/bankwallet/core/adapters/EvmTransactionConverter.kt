...
class EvmTransactionConverter(
            ...
            when (val decoration = fullTransaction.decoration) {
                ...
                is OneInchSwapDecoration -> {
                    SwapTransactionRecord(
                        ...
                        source,
                        decoration.contractAddress.eip55,
                        SwapTransactionRecord.Amount.Exact(
    ...
    private fun convertToAmount(
        ...
        when (amount) {
            ...
            is SwapDecoration.Amount.Extremum ->
                SwapTransactionRecord.Amount.Extremum(
                    convertToTransactionValue(token, amount.value, negative),
                )
        }
    ...
    private fun convertToAmount(
        ...
        when (amount) {
            is OneInchDecoration.Amount.Exact ->
                SwapTransactionRecord.Amount.Exact(
                    convertToTransactionValue(token, amount.value, negative),
                )

            is OneInchDecoration.Amount.Extremum ->
                SwapTransactionRecord.Amount.Extremum(
                    convertToTransactionValue(token, amount.value, negative),
                )
        }
