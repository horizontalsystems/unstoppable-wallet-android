...
class NearNetworkViewModel(private val service: NearNetworkService) : ViewModelUiState<NearNetworkViewModel.UiState>() {

    private var closeScreen = false
    private var viewItems = listOf<ViewItem>()

    val title: String = "NEAR"
    val blockchainType = BlockchainType.Near
    ...
    init {
        viewModelScope.launch {
            service.stateFlow
                .catch { Timber.e(it, "NEAR network items collection failed") }
                .collect { state ->
    ...
    private fun viewItem(item: NearNetworkService.Item) = ViewItem(
